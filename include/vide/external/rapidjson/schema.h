...
#ifndef VIDE_RAPIDJSON_SCHEMA_H_
#define VIDE_RAPIDJSON_SCHEMA_H_
...
#if !defined(VIDE_RAPIDJSON_SCHEMA_USE_INTERNALREGEX)
#define VIDE_RAPIDJSON_SCHEMA_USE_INTERNALREGEX 1
#endif

#if !defined(VIDE_RAPIDJSON_SCHEMA_USE_STDREGEX) || !(__cplusplus >=201103L || (defined(_MSC_VER) && _MSC_VER >= 1800))
#define VIDE_RAPIDJSON_SCHEMA_USE_STDREGEX 0
#endif

#if VIDE_RAPIDJSON_SCHEMA_USE_INTERNALREGEX
#include "internal/regex.h"
#elif VIDE_RAPIDJSON_SCHEMA_USE_STDREGEX
#include <regex>
#endif

#if VIDE_RAPIDJSON_SCHEMA_USE_INTERNALREGEX || VIDE_RAPIDJSON_SCHEMA_USE_STDREGEX
#define VIDE_RAPIDJSON_SCHEMA_HAS_REGEX 1
#else
#define VIDE_RAPIDJSON_SCHEMA_HAS_REGEX 0
#endif

#ifndef VIDE_RAPIDJSON_SCHEMA_VERBOSE
#define VIDE_RAPIDJSON_SCHEMA_VERBOSE 0
#endif
...
#ifdef __clang__
VIDE_RAPIDJSON_DIAG_OFF(weak-vtables)
VIDE_RAPIDJSON_DIAG_OFF(exit-time-destructors)
VIDE_RAPIDJSON_DIAG_OFF(c++98-compat-pedantic)
VIDE_RAPIDJSON_DIAG_OFF(variadic-macros)
#elif defined(_MSC_VER)
VIDE_RAPIDJSON_DIAG_OFF(4512) // assignment operator could not be generated
#endif
...
#if VIDE_RAPIDJSON_SCHEMA_VERBOSE
...
#endif // VIDE_RAPIDJSON_SCHEMA_VERBOSE

#ifndef VIDE_RAPIDJSON_SCHEMA_PRINT
#if VIDE_RAPIDJSON_SCHEMA_VERBOSE
#define VIDE_RAPIDJSON_SCHEMA_PRINT(name, ...) internal::Print##name##Data(__VA_ARGS__)
#else
#define VIDE_RAPIDJSON_SCHEMA_PRINT(name, ...)
#endif
#endif

///////////////////////////////////////////////////////////////////////////////
// VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN

#define VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(code)\
VIDE_RAPIDJSON_MULTILINEMACRO_BEGIN\
    context.invalidCode = code;\
    context.invalidKeyword = SchemaType::GetValidateErrorKeyword(code).GetString();\
    VIDE_RAPIDJSON_SCHEMA_PRINT(InvalidKeyword, context.invalidKeyword);\
    return false;\
VIDE_RAPIDJSON_MULTILINEMACRO_END

///////////////////////////////////////////////////////////////////////////////
// ValidateFlag

/*! \def VIDE_RAPIDJSON_VALIDATE_DEFAULT_FLAGS
...
*/
#ifndef VIDE_RAPIDJSON_VALIDATE_DEFAULT_FLAGS
#define VIDE_RAPIDJSON_VALIDATE_DEFAULT_FLAGS kValidateNoFlags
#endif
...
enum ValidateFlag {
    ...
    kValidateDefaultFlags = VIDE_RAPIDJSON_VALIDATE_DEFAULT_FLAGS    //!< Default validate flags. Can be customized by defining VIDE_RAPIDJSON_VALIDATE_DEFAULT_FLAGS
};
...
private:
    ...
    bool WriteBuffer(Type type, const void* data, size_t len) {
        ...
        uint64_t h = Hash(VIDE_RAPIDJSON_UINT64_C2(0xcbf29ce4, 0x84222325), type);
    ...
    static uint64_t Hash(uint64_t h, uint64_t d) {
        static const uint64_t kPrime = VIDE_RAPIDJSON_UINT64_C2(0x00000100, 0x000001b3);
        h ^= d;
        h *= kPrime;
        return h;
    }
...
public:
        ...
        nullable_(false)
    {
        ...
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Schema", sb.GetString(), id.GetString());
        ...
        if (const ValueType* v = GetMember(value, GetIdString())) {
            if (v->IsString()) {
                ...
                id_ = local.Resolve(id_, allocator);
                    VIDE_RAPIDJSON_SCHEMA_PRINT(SchemaIds, id.GetString(), v->GetString(), id_.GetString());
            }
        }
...
#if VIDE_RAPIDJSON_SCHEMA_HAS_REGEX
        if (pattern_) {
            pattern_->~RegexType();
    ...
    bool BeginValue(Context& context) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::BeginValue");
        if (context.inArray) {
            if (uniqueItems_)
            ...
            if (itemsList_)
                ...
                if (context.arrayElementIndex < itemsTupleCount_)
                    ...
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorAdditionalItems);
                }
            }
            else
                context.valueSchema = typeless_;
    ...
    VIDE_RAPIDJSON_FORCEINLINE bool EndValue(Context& context) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::EndValue");
        ...
        if (context.patternPropertiesValidatorCount > 0) {
            ...
            if (context.objectPatternValidatorType == Context::kPatternValidatorOnly) {
                if (!patternValid) {
                    ...
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorPatternProperties);
                }
            }
        ...
        if (enum_ && context.hasher) {
            ...
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorEnum);
            foundEnum:;
        }
        ...
        if (context.validatorCount > 0) {
            if (allOf_.schemas)
                for (SizeType i = allOf_.begin; i < allOf_.begin + allOf_.count; i++)
                    if (!context.validators[i]->IsValid()) {
                        ...
                        VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorAllOf);
                    }
            ...
            if (anyOf_.schemas) {
                ...
                VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorAnyOf);
                foundAny:;
            }
            ...
            if (oneOf_.schemas) {
                ...
                for (SizeType i = oneOf_.begin; i < oneOf_.begin + oneOf_.count; i++)
                    if (context.validators[i]->IsValid()) {
                        if (oneValid) {
                            ...
                            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorOneOfMatch);
                        } else {
                            oneValid = true;
                ...
                if (!oneValid) {
                    ...
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorOneOf);
                }
            }

            if (not_ && context.validators[notValidatorIndex_]->IsValid()) {
                context.error_handler.Disallowed();
                VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorNot);
            }
        }
    ...
    bool Null(Context& context) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Null");
        if (!(type_ & (1 << kNullSchemaType))) {
            DisallowedType(context, GetNullString());
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorType);
        }
        return CreateParallelValidator(context);
    }

    bool Bool(Context& context, bool b) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Bool", b);
    ...
    bool Int(Context& context, int i) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Int", (int64_t)i);
    ...
    bool Uint(Context& context, unsigned u) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Uint", (uint64_t)u);
    ...
    bool Int64(Context& context, int64_t i) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Int64", i);
        if (!CheckInt(context, i))
            return false;
        return CreateParallelValidator(context);
    }
    ...
    bool Uint64(Context& context, uint64_t u) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Uint64", u);
        if (!CheckUint(context, u))
            return false;
        return CreateParallelValidator(context);
    }
    ...
    bool Double(Context& context, double d) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Double", d);
    ...
    bool String(Context& context, const Ch* str, SizeType length, bool) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::String", str);
        ...
        if (minLength_ != 0 || maxLength_ != SizeType(~0)) {
            ...
            if (internal::CountStringCodePoint<EncodingType>(str, length, &count)) {
                if (count < minLength_) {
                    ...
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorMinLength);
                }
                if (count > maxLength_) {
                    context.error_handler.TooLong(str, length, maxLength_);
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorMaxLength);
                }
            }
        }
        ...
        if (pattern_ && !IsPatternMatch(pattern_, str, length)) {
            ...
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorPattern);
        }
    ...
    bool StartObject(Context& context) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::StartObject");
    ...
    bool Key(Context& context, const Ch* str, SizeType len, bool) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Key", str);
        ...
        if (context.patternPropertiesSchemaCount == 0) { // patternProperties are not additional properties
            ...
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorAdditionalProperties);
        }
    ...
    bool EndObject(Context& context, SizeType memberCount) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::EndObject");
        if (hasRequired_) {
            ...
            if (context.error_handler.EndMissingProperties())
                VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorRequired);
        }

        if (memberCount < minProperties_) {
            context.error_handler.TooFewProperties(memberCount, minProperties_);
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorMinProperties);
        }

        if (memberCount > maxProperties_) {
            context.error_handler.TooManyProperties(memberCount, maxProperties_);
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorMaxProperties);
        }
        ...
        if (hasDependencies_) {
            ...
            if (context.error_handler.EndDependencyErrors())
                VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorDependencies);
        }
    ...
    bool StartArray(Context& context) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::StartArray");
    ...
    bool EndArray(Context& context, SizeType elementCount) const {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "Schema::EndArray");
        context.inArray = false;

        if (elementCount < minItems_) {
            context.error_handler.TooFewItems(elementCount, minItems_);
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorMinItems);
        }

        if (elementCount > maxItems_) {
            context.error_handler.TooManyItems(elementCount, maxItems_);
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorMaxItems);
        }
...
#define VIDE_RAPIDJSON_STRING_(name, ...) \
    ...
    VIDE_RAPIDJSON_STRING_(Null, 'n', 'u', 'l', 'l')
    VIDE_RAPIDJSON_STRING_(Boolean, 'b', 'o', 'o', 'l', 'e', 'a', 'n')
    VIDE_RAPIDJSON_STRING_(Object, 'o', 'b', 'j', 'e', 'c', 't')
    VIDE_RAPIDJSON_STRING_(Array, 'a', 'r', 'r', 'a', 'y')
    VIDE_RAPIDJSON_STRING_(String, 's', 't', 'r', 'i', 'n', 'g')
    VIDE_RAPIDJSON_STRING_(Number, 'n', 'u', 'm', 'b', 'e', 'r')
    VIDE_RAPIDJSON_STRING_(Integer, 'i', 'n', 't', 'e', 'g', 'e', 'r')
    VIDE_RAPIDJSON_STRING_(Type, 't', 'y', 'p', 'e')
    VIDE_RAPIDJSON_STRING_(Enum, 'e', 'n', 'u', 'm')
    VIDE_RAPIDJSON_STRING_(AllOf, 'a', 'l', 'l', 'O', 'f')
    VIDE_RAPIDJSON_STRING_(AnyOf, 'a', 'n', 'y', 'O', 'f')
    VIDE_RAPIDJSON_STRING_(OneOf, 'o', 'n', 'e', 'O', 'f')
    VIDE_RAPIDJSON_STRING_(Not, 'n', 'o', 't')
    VIDE_RAPIDJSON_STRING_(Properties, 'p', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's')
    VIDE_RAPIDJSON_STRING_(Required, 'r', 'e', 'q', 'u', 'i', 'r', 'e', 'd')
    VIDE_RAPIDJSON_STRING_(Dependencies, 'd', 'e', 'p', 'e', 'n', 'd', 'e', 'n', 'c', 'i', 'e', 's')
    VIDE_RAPIDJSON_STRING_(PatternProperties, 'p', 'a', 't', 't', 'e', 'r', 'n', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's')
    VIDE_RAPIDJSON_STRING_(AdditionalProperties, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n', 'a', 'l', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's')
    VIDE_RAPIDJSON_STRING_(MinProperties, 'm', 'i', 'n', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's')
    VIDE_RAPIDJSON_STRING_(MaxProperties, 'm', 'a', 'x', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's')
    VIDE_RAPIDJSON_STRING_(Items, 'i', 't', 'e', 'm', 's')
    VIDE_RAPIDJSON_STRING_(MinItems, 'm', 'i', 'n', 'I', 't', 'e', 'm', 's')
    VIDE_RAPIDJSON_STRING_(MaxItems, 'm', 'a', 'x', 'I', 't', 'e', 'm', 's')
    VIDE_RAPIDJSON_STRING_(AdditionalItems, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n', 'a', 'l', 'I', 't', 'e', 'm', 's')
    VIDE_RAPIDJSON_STRING_(UniqueItems, 'u', 'n', 'i', 'q', 'u', 'e', 'I', 't', 'e', 'm', 's')
    VIDE_RAPIDJSON_STRING_(MinLength, 'm', 'i', 'n', 'L', 'e', 'n', 'g', 't', 'h')
    VIDE_RAPIDJSON_STRING_(MaxLength, 'm', 'a', 'x', 'L', 'e', 'n', 'g', 't', 'h')
    VIDE_RAPIDJSON_STRING_(Pattern, 'p', 'a', 't', 't', 'e', 'r', 'n')
    VIDE_RAPIDJSON_STRING_(Minimum, 'm', 'i', 'n', 'i', 'm', 'u', 'm')
    VIDE_RAPIDJSON_STRING_(Maximum, 'm', 'a', 'x', 'i', 'm', 'u', 'm')
    VIDE_RAPIDJSON_STRING_(ExclusiveMinimum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'i', 'n', 'i', 'm', 'u', 'm')
    VIDE_RAPIDJSON_STRING_(ExclusiveMaximum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'a', 'x', 'i', 'm', 'u', 'm')
    VIDE_RAPIDJSON_STRING_(MultipleOf, 'm', 'u', 'l', 't', 'i', 'p', 'l', 'e', 'O', 'f')
    VIDE_RAPIDJSON_STRING_(DefaultValue, 'd', 'e', 'f', 'a', 'u', 'l', 't')
    VIDE_RAPIDJSON_STRING_(Schema, '$', 's', 'c', 'h', 'e', 'm', 'a')
    VIDE_RAPIDJSON_STRING_(Ref, '$', 'r', 'e', 'f')
    VIDE_RAPIDJSON_STRING_(Id, 'i', 'd')
    VIDE_RAPIDJSON_STRING_(Swagger, 's', 'w', 'a', 'g', 'g', 'e', 'r')
    VIDE_RAPIDJSON_STRING_(OpenApi, 'o', 'p', 'e', 'n', 'a', 'p', 'i')
    VIDE_RAPIDJSON_STRING_(ReadOnly, 'r', 'e', 'a', 'd', 'O', 'n', 'l', 'y')
    VIDE_RAPIDJSON_STRING_(WriteOnly, 'w', 'r', 'i', 't', 'e', 'O', 'n', 'l', 'y')
    VIDE_RAPIDJSON_STRING_(Nullable, 'n', 'u', 'l', 'l', 'a', 'b', 'l', 'e')

#undef VIDE_RAPIDJSON_STRING_
...
private:
...
#endif // VIDE_RAPIDJSON_SCHEMA_USE_STDREGEX
    ...
    bool CreateParallelValidator(Context& context) const {
        ...
        if (validatorCount_) {
            VIDE_RAPIDJSON_ASSERT(context.validators == 0);
        ...
        if (readOnly_ && (context.flags & kValidateWriteFlag)) {
            ...
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorReadOnly);
        }
        if (writeOnly_ && (context.flags & kValidateReadFlag)) {
            context.error_handler.DisallowedWhenReading();
            VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorWriteOnly);
        }
        ...
        if (!minimum_.IsNull()) {
            if (minimum_.IsInt64()) {
                if (exclusiveMinimum_ ? i <= minimum_.GetInt64() : i < minimum_.GetInt64()) {
                    ...
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(exclusiveMinimum_ ? kValidateErrorExclusiveMinimum : kValidateErrorMinimum);
                }
            }
            else if (minimum_.IsUint64()) {
                context.error_handler.BelowMinimum(i, minimum_, exclusiveMinimum_);
                VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(exclusiveMinimum_ ? kValidateErrorExclusiveMinimum : kValidateErrorMinimum); // i <= max(int64_t) < minimum.GetUint64()
            }
        ...
        if (!maximum_.IsNull()) {
            if (maximum_.IsInt64()) {
                if (exclusiveMaximum_ ? i >= maximum_.GetInt64() : i > maximum_.GetInt64()) {
                    ...
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(exclusiveMaximum_ ? kValidateErrorExclusiveMaximum : kValidateErrorMaximum);
                }
            }
        ...
        if (!multipleOf_.IsNull()) {
            if (multipleOf_.IsUint64()) {
                if (static_cast<uint64_t>(i >= 0 ? i : -i) % multipleOf_.GetUint64() != 0) {
                    ...
                    VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorMultipleOf);
                }
            }
    ...
    bool CheckUint(Context& context, uint64_t i) const {
        ...
        if (!maximum_.IsNull()) {
            if (maximum_.IsUint64()) {
                ...
                context.error_handler.AboveMaximum(i, maximum_, exclusiveMaximum_);
                VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(exclusiveMaximum_ ? kValidateErrorExclusiveMaximum : kValidateErrorMaximum); // i >= 0 > maximum_
            }
            else if (!CheckDoubleMaximum(context, static_cast<double>(i)))
                return false;
        }
...
struct TokenHelper {
    VIDE_RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack& documentStack, SizeType index) {
...
struct TokenHelper<Stack, char> {
    VIDE_RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack& documentStack, SizeType index) {
        VIDE_RAPIDJSON_IF_CONSTEXPR (sizeof(SizeType) == 4) {
...
public:
    ...
    */
    explicit GenericSchemaDocument(const ValueType& document, const Ch* uri = 0, SizeType uriLength = 0,
        ...
        currentError_()
    {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::GenericSchemaDocument");
        if (!allocator_)
    ...
    GenericSchemaDocument(GenericSchemaDocument&& rhs) VIDE_RAPIDJSON_NOEXCEPT :
  ...
  private:
    ...
    void AddCurrentError(const SchemaErrorCode code, const PointerType& location) {
      VIDE_RAPIDJSON_SCHEMA_PRINT(InvalidKeyword, GetSchemaErrorKeyword(code));
...
#define VIDE_RAPIDJSON_STRING_(name, ...) \
    ...
    VIDE_RAPIDJSON_STRING_(InstanceRef, 'i', 'n', 's', 't', 'a', 'n', 'c', 'e', 'R', 'e', 'f')
    VIDE_RAPIDJSON_STRING_(ErrorCode, 'e', 'r', 'r', 'o', 'r', 'C', 'o', 'd', 'e')
    VIDE_RAPIDJSON_STRING_(Value, 'v', 'a', 'l', 'u', 'e')
    VIDE_RAPIDJSON_STRING_(Offset, 'o', 'f', 'f', 's', 'e', 't')

    VIDE_RAPIDJSON_STRING_(Null, 'n', 'u', 'l', 'l')
    VIDE_RAPIDJSON_STRING_(SpecUnknown, 'S', 'p', 'e', 'c', 'U', 'n', 'k', 'n', 'o', 'w', 'n')
    VIDE_RAPIDJSON_STRING_(SpecUnsupported, 'S', 'p', 'e', 'c', 'U', 'n', 's', 'u', 'p', 'p', 'o', 'r', 't', 'e', 'd')
    VIDE_RAPIDJSON_STRING_(SpecIllegal, 'S', 'p', 'e', 'c', 'I', 'l', 'l', 'e', 'g', 'a', 'l')
    VIDE_RAPIDJSON_STRING_(StartUnknown, 'S', 't', 'a', 'r', 't', 'U', 'n', 'k', 'n', 'o', 'w', 'n')
    VIDE_RAPIDJSON_STRING_(RefPlainName, 'R', 'e', 'f', 'P', 'l', 'a', 'i', 'n', 'N', 'a', 'm', 'e')
    VIDE_RAPIDJSON_STRING_(RefInvalid, 'R', 'e', 'f', 'I', 'n', 'v', 'a', 'l', 'i', 'd')
    VIDE_RAPIDJSON_STRING_(RefPointerInvalid, 'R', 'e', 'f', 'P', 'o', 'i', 'n', 't', 'e', 'r', 'I', 'n', 'v', 'a', 'l', 'i', 'd')
    VIDE_RAPIDJSON_STRING_(RefUnknown, 'R', 'e', 'f', 'U', 'n', 'k', 'n', 'o', 'w', 'n')
    VIDE_RAPIDJSON_STRING_(RefCyclical, 'R', 'e', 'f', 'C', 'y', 'c', 'l', 'i', 'c', 'a', 'l')
    VIDE_RAPIDJSON_STRING_(RefNoRemoteProvider, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm', 'o', 't', 'e', 'P', 'r', 'o', 'v', 'i', 'd', 'e', 'r')
    VIDE_RAPIDJSON_STRING_(RefNoRemoteSchema, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm', 'o', 't', 'e', 'S', 'c', 'h', 'e', 'm', 'a')
    VIDE_RAPIDJSON_STRING_(ReadOnlyAndWriteOnly, 'R', 'e', 'a', 'd', 'O', 'n', 'l', 'y', 'A', 'n', 'd', 'W', 'r', 'i', 't', 'e', 'O', 'n', 'l', 'y')
    VIDE_RAPIDJSON_STRING_(RegexInvalid, 'R', 'e', 'g', 'e', 'x', 'I', 'n', 'v', 'a', 'l', 'i', 'd')
...
#undef VIDE_RAPIDJSON_STRING_
    ...
    const UriType& CreateSchema(const SchemaType** schema, const PointerType& pointer, const ValueType& v, const ValueType& document, const UriType& id) {
        VIDE_RAPIDJSON_ASSERT(pointer.IsValid());
        GenericStringBuffer<EncodingType> sb;
        pointer.StringifyUriFragment(sb);
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::CreateSchema", sb.GetString(), id.GetString());
        if (v.IsObject()) {
    ...
    bool HandleRefSchema(const PointerType& source, const SchemaType** schema, const ValueType& v, const ValueType& document, const UriType& id) {
        ...
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::HandleRefSchema", sb.GetString(), id.GetString());
        ...
        if (itr->value.IsString()) {
            ...
            if (len == 0)
                ...
                VIDE_RAPIDJSON_SCHEMA_PRINT(SchemaIds, id.GetString(), itr->value.GetString(), ref.GetString());
    ...
    ValueType* FindId(const ValueType& doc, const UriType& finduri, PointerType& resptr, const UriType& baseuri, bool full, const PointerType& here = PointerType()) const {
        ...
        if (doc.GetType() == kObjectType) {
            ...
            if (localuri.Match(finduri, full)) {
                VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::FindId (match)", full ? localuri.GetString() : localuri.GetBaseString());
    ...
    void AddSchemaRefs(SchemaType* schema) {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::AddSchemaRefs");
...
public:
    ...
    */
    GenericSchemaValidator(
        ...
        depth_(0)
    {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::GenericSchemaValidator");
    }
    ...
    */
    GenericSchemaValidator(
        ...
        depth_(0)
    {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::GenericSchemaValidator (output handler)");
    }

    //! Destructor.
    ~GenericSchemaValidator() {
        Reset();
        VIDE_RAPIDJSON_DELETE(ownStateAllocator_);
    }
...
#define VIDE_RAPIDJSON_STRING_(name, ...) \
    ...
    VIDE_RAPIDJSON_STRING_(InstanceRef, 'i', 'n', 's', 't', 'a', 'n', 'c', 'e', 'R', 'e', 'f')
    VIDE_RAPIDJSON_STRING_(SchemaRef, 's', 'c', 'h', 'e', 'm', 'a', 'R', 'e', 'f')
    VIDE_RAPIDJSON_STRING_(Expected, 'e', 'x', 'p', 'e', 'c', 't', 'e', 'd')
    VIDE_RAPIDJSON_STRING_(Actual, 'a', 'c', 't', 'u', 'a', 'l')
    VIDE_RAPIDJSON_STRING_(Disallowed, 'd', 'i', 's', 'a', 'l', 'l', 'o', 'w', 'e', 'd')
    VIDE_RAPIDJSON_STRING_(Missing, 'm', 'i', 's', 's', 'i', 'n', 'g')
    VIDE_RAPIDJSON_STRING_(Errors, 'e', 'r', 'r', 'o', 'r', 's')
    VIDE_RAPIDJSON_STRING_(ErrorCode, 'e', 'r', 'r', 'o', 'r', 'C', 'o', 'd', 'e')
    VIDE_RAPIDJSON_STRING_(ErrorMessage, 'e', 'r', 'r', 'o', 'r', 'M', 'e', 's', 's', 'a', 'g', 'e')
    VIDE_RAPIDJSON_STRING_(Duplicates, 'd', 'u', 'p', 'l', 'i', 'c', 'a', 't', 'e', 's')
    VIDE_RAPIDJSON_STRING_(Matches, 'm', 'a', 't', 'c', 'h', 'e', 's')

#undef VIDE_RAPIDJSON_STRING_

#define VIDE_RAPIDJSON_SCHEMA_HANDLE_BEGIN_(method, arg1)\
    if (!valid_) return false; \
    if ((!BeginValue() && !GetContinueOnErrors()) || (!CurrentSchema().method arg1 && !GetContinueOnErrors())) {\
        *documentStack_.template Push<Ch>() = '\0';\
        documentStack_.template Pop<Ch>(1);\
        VIDE_RAPIDJSON_SCHEMA_PRINT(InvalidDocument, documentStack_.template Bottom<Ch>());\
        valid_ = false;\
        return valid_;\
    }

#define VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(method, arg2)\
...
#define VIDE_RAPIDJSON_SCHEMA_HANDLE_END_(method, arg2)\
    valid_ = (EndValue() || GetContinueOnErrors()) && (!outputHandler_ || outputHandler_->method arg2);\
    return valid_;

#define VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(method, arg1, arg2) \
    VIDE_RAPIDJSON_SCHEMA_HANDLE_BEGIN_   (method, arg1);\
    VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(method, arg2);\
    VIDE_RAPIDJSON_SCHEMA_HANDLE_END_     (method, arg2)

    bool Null()             { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(Null,   (CurrentContext()), ( )); }
    bool Bool(bool b)       { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(Bool,   (CurrentContext(), b), (b)); }
    bool Int(int i)         { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(Int,    (CurrentContext(), i), (i)); }
    bool Uint(unsigned u)   { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(Uint,   (CurrentContext(), u), (u)); }
    bool Int64(int64_t i)   { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(Int64,  (CurrentContext(), i), (i)); }
    bool Uint64(uint64_t u) { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(Uint64, (CurrentContext(), u), (u)); }
    bool Double(double d)   { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(Double, (CurrentContext(), d), (d)); }
    bool RawNumber(const Ch* str, SizeType length, bool copy)
                                    { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(String, (CurrentContext(), str, length, copy), (str, length, copy)); }
    bool String(const Ch* str, SizeType length, bool copy)
                                    { VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_(String, (CurrentContext(), str, length, copy), (str, length, copy)); }

    bool StartObject() {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::StartObject");
        VIDE_RAPIDJSON_SCHEMA_HANDLE_BEGIN_(StartObject, (CurrentContext()));
        VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(StartObject, ());
        valid_ = !outputHandler_ || outputHandler_->StartObject();
        return valid_;
    }

    bool Key(const Ch* str, SizeType len, bool copy) {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::Key", str);
        if (!valid_) return false;
        AppendToken(str, len);
        ...
        VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(Key, (str, len, copy));
        valid_ = !outputHandler_ || outputHandler_->Key(str, len, copy);
        return valid_;
    }

    bool EndObject(SizeType memberCount) {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::EndObject");
        if (!valid_) return false;
        VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(EndObject, (memberCount));
        if (!CurrentSchema().EndObject(CurrentContext(), memberCount) && !GetContinueOnErrors()) {
            valid_ = false;
            return valid_;
        }
        VIDE_RAPIDJSON_SCHEMA_HANDLE_END_(EndObject, (memberCount));
    }

    bool StartArray() {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::StartArray");
        VIDE_RAPIDJSON_SCHEMA_HANDLE_BEGIN_(StartArray, (CurrentContext()));
        VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(StartArray, ());
        valid_ = !outputHandler_ || outputHandler_->StartArray();
        return valid_;
    }

    bool EndArray(SizeType elementCount) {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::EndArray");
        if (!valid_) return false;
        VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(EndArray, (elementCount));
        if (!CurrentSchema().EndArray(CurrentContext(), elementCount) && !GetContinueOnErrors()) {
            valid_ = false;
            return valid_;
        }
        VIDE_RAPIDJSON_SCHEMA_HANDLE_END_(EndArray, (elementCount));
    }

#undef VIDE_RAPIDJSON_SCHEMA_HANDLE_BEGIN_
#undef VIDE_RAPIDJSON_SCHEMA_HANDLE_PARALLEL_
#undef VIDE_RAPIDJSON_SCHEMA_HANDLE_VALUE_
...
private:
    ...
    GenericSchemaValidator(
        ...
        depth_(depth)
    {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::GenericSchemaValidator (internal)", basePath && basePathSize ? basePath : "");
    ...
    StateAllocator& GetStateAllocator() {
        if (!stateAllocator_)
            stateAllocator_ = ownStateAllocator_ = VIDE_RAPIDJSON_NEW(StateAllocator)();
    ...
    bool BeginValue() {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::BeginValue");
        if (schemaStack_.Empty())
            ...
            VIDE_RAPIDJSON_ASSERT(CurrentContext().valueSchema);
    ...
    bool EndValue() {
        VIDE_RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::EndValue");
        ...
        *documentStack_.template Push<Ch>() = '\0';
        documentStack_.template Pop<Ch>(1);
        VIDE_RAPIDJSON_SCHEMA_PRINT(ValidatorPointers, sb.GetString(), documentStack_.template Bottom<Ch>(), depth_);
            ...
            if (hasher && context.valueUniqueness) {
                ...
                for (typename HashCodeArray::ConstValueIterator itr = a->Begin(); itr != a->End(); ++itr)
                    if (itr->GetUint64() == h) {
                        ...
                        VIDE_RAPIDJSON_INVALID_KEYWORD_RETURN(kValidateErrorUniqueItems);
                    }
                a->PushBack(h, GetStateAllocator());
            }
        }
    ...
    VIDE_RAPIDJSON_FORCEINLINE void PushSchema(const SchemaType& schema) { new (schemaStack_.template Push<Context>()) Context(*this, *this, &schema, flags_); }

    VIDE_RAPIDJSON_FORCEINLINE void PopSchema() {
...
#endif // VIDE_RAPIDJSON_SCHEMA_H_
